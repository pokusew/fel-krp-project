...
static_assert(AES256 == 1);
static_assert(AES_KEYLEN == 32);
static_assert(AES_BLOCKLEN == 16);

// The current test vectors taken from NIST SP 800-38A
// Recommendation for Block Cipher Modes of Operation: Methods and Techniques
// (https://doi.org/10.6028/NIST.SP.800-38A),
// sections F.2.5 CBC-AES256.Encrypt and F.2.6 CBC-AES256.Decrypt (pages 28-29).
...
>();
static_assert(key.size() == AES_KEYLEN);

constexpr auto iv = hex::bytes<
	"000102030405060708090a0b0c0d0e0f"
>();
static_assert(iv.size() == AES_BLOCKLEN);
...
>();
static_assert(plaintext.size() % AES_BLOCKLEN == 0);
...
>();
static_assert(ciphertext.size() % AES_BLOCKLEN == 0);
...
TEST(TinyAesTest, Aes256CbcEncrypt) {
	...
	EXPECT_SAME_BYTES_S(AES_BLOCKLEN, aes_ctx.Iv, &ciphertext[ciphertext.size() - AES_BLOCKLEN]);
}
