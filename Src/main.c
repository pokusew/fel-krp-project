...
static void MX_TIM2_Init(void) {
	...
	// TIM2 is on APB1, so it uses APB1 clock, which is currently configured to 84 MHz
	// f_TIM2 = f_CK_APB1 / (prescaler + 1)
	// f_TIM2 = 84_000_000 Hz / (83 + 1) = 1_000_000 Hz = 1 MHz
	// max counter value 4294967295 is in microseconds (us) which is ~ 4294 seconds ~ 71 minutes,
	// so we can the current TIM2 counter value as an absolute time during development
