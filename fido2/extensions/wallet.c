...
int8_t wallet_pin(uint8_t subcmd, uint8_t *pinAuth, uint8_t *arg1, uint8_t *arg2, uint8_t *arg3, int len) {
	...
	switch (subcmd) {
		case CP_cmdGetKeyAgreement:
			...
			printf1(TAG_WALLET, "pubkey: ");
			dump_hex1(TAG_WALLET, KEY_AGREEMENT_PUB, 64);
...
//            printf1(TAG_WALLET,"Success.  Pin = %s\n", STATE.pin_code);
		...
		case CP_cmdGetPinToken:
			...
			printf1(TAG_WALLET, "pinToken: ");
			dump_hex1(TAG_WALLET, PIN_TOKEN, 16);
...
int16_t bridge_to_wallet(uint8_t *keyh, uint8_t klen) {
	...
	printf1(TAG_WALLET, "u2f2wallet [%d]: ", reqlen);
	dump_hex1(TAG_WALLET, msg_buf, reqlen);
	...
	switch (req->operation) {
		case WalletSign:
			...
			printf1(TAG_WALLET, "pinAuth:");
			dump_hex1(TAG_WALLET, req->pinAuth, 16);
			...
			printf1(TAG_WALLET, "challenge:");
			dump_hex1(TAG_WALLET, args[0], lens[0]);
			if (args[1] != NULL && req->numArgs > 1) {
				printf1(TAG_WALLET, "keyid is specified\r\n");
				printf1(TAG_WALLET, "keyid:");
				dump_hex1(TAG_WALLET, args[1], lens[1]);
			}
		...
		case WalletRegister:
			...
			printf1(TAG_WALLET, "adding key [%d]: ", lens[0]);
			dump_hex1(TAG_WALLET, args[0], lens[0]);
