...
static uint8_t verify_pin_auth_ex(uint8_t *pinAuth, uint8_t *buf, size_t len) {
	...
	if (memcmp(pinAuth, hmac, 16) == 0) {
		...
		printf2(TAG_ERR, "Pin auth failed" nl);
...
static int
ctap_generate_cose_key(CborEncoder *cose_key, uint8_t *hmac_input, int len, uint8_t credtype, int32_t algtype) {
	uint8_t x[32], y[32];

	if (credtype != PUB_KEY_CRED_PUB_KEY) {
		printf2(TAG_ERR, "Error, pubkey credential type not supported" nl);
		return -1;
	}
	switch (algtype) {
		case COSE_ALG_ES256:
		...
		case COSE_ALG_EDDSA:
			...
			break;
		default:
			printf2(TAG_ERR, "Error, COSE alg %d not supported" nl, algtype);
			return -1;
	}
...
static int ctap_make_extensions(CTAP_extensions *ext, uint8_t *ext_encoder_buf, unsigned int *ext_encoder_buf_size) {
	...
	if (ext->hmac_secret_present == EXT_HMAC_SECRET_PARSED) {
		printf1(TAG_CTAP, "Processing hmac-secret.." nl);
		...
		if (memcmp(ext->hmac_secret.saltAuth, hmac, 16) == 0) {
			printf1(TAG_CTAP, "saltAuth is valid" nl);
		} else {
			printf1(TAG_CTAP, "saltAuth is invalid" nl);
...
ctap_make_auth_data(struct rpId *rp, CborEncoder *map, uint8_t *auth_data_buf, uint32_t *len, CTAP_credInfo *credInfo,
	...
	if ((sizeof(CTAP_authDataHeader)) > *len) {
		printf1(TAG_ERR, "assertion fail, auth_data_buf must be at least %d bytes" nl,
	...
	if (credInfo != NULL) {
		...
		if (credInfo->rk) {
			...
			printf2(TAG_ERR, "Out of memory for resident keys" nl);
...
// @return length of der signature
int ctap_calculate_signature(uint8_t *data, int datalen, uint8_t *clientDataHash, uint8_t *hashbuf, uint8_t *sigbuf,
							 uint8_t *sigder, int32_t alg) {
	// calculate attestation sig
	if (alg == COSE_ALG_EDDSA) {
		crypto_ed25519_sign(data, datalen, clientDataHash, CLIENT_DATA_HASH_SIZE,
...
uint8_t ctap_add_attest_statement(CborEncoder *map, uint8_t *sigder, int len) {
	...
	if (cert_size > sizeof(cert)) {
		printf2(TAG_ERR, "Certificate is too large for CTAP2 buffer" nl);
		return CTAP2_ERR_PROCESSING;
	}
...
int ctap_authenticate_credential(struct rpId *rp, CTAP_credentialDescriptor *desc) {
	...
	switch (desc->type) {
		case PUB_KEY_CRED_PUB_KEY:
			...
			printf1(TAG_RED, "rpId: %s" nl, rp->id);
		...
		case PUB_KEY_CRED_CTAP1:
			...
			// 	break;
		default:
			printf1(TAG_ERR, "PUB_KEY_CRED_UNKNOWN %x" nl, desc->type);
			break;
	}
...
uint8_t ctap_make_credential(CborEncoder *encoder, uint8_t *request, int length) {
	...
	if (ret != 0) {
		printf2(TAG_ERR, "error, parse_make_credential failed" nl);
		return ret;
	}
	if (MC.pinAuthEmpty) {
	...
	if ((MC.paramsParsed & MC_requiredMask) != MC_requiredMask) {
		printf2(TAG_ERR, "error, required parameter(s) for makeCredential are missing" nl);
		return CTAP2_ERR_MISSING_PARAMETER;
	}

	if (ctap_is_pin_set() == 1 && MC.pinAuthPresent == 0) {
		printf2(TAG_ERR, "pinAuth is required" nl);
	...
	for (i = 0; i < MC.excludeListSize; i++) {
		...
		if (ctap_authenticate_credential(&MC.rp, excl_cred)) {
			if (check_credential_metadata(&excl_cred->credential.id, MC.pinAuthPresent, 1) == 0) {
				ret = ctap2_user_presence_test();
				check_retr(ret);
				printf1(TAG_MC, "Cred %d failed!" nl, i);
...
static int add_existing_user_info(CTAP_credentialDescriptor *cred) {
	...
	for (i = 0; i < index; i++) {
		...
		if (is_cred_id_matching_rk(&cred->credential.id, &rk)) {
			printf1(TAG_GREEN, "found rk match for allowList item (%d)" nl, i);
			memmove(&cred->credential.user, &rk.user, sizeof(CTAP_userEntity));
			return 1;
		}

	}
	printf1(TAG_GREEN, "NO rk match for allowList item " nl);
	return 0;
}
...
int ctap_filter_invalid_credentials(CTAP_getAssertion *GA) {
	...
	for (i = 0; i < (unsigned int) GA->credLen; i++) {
		if (!ctap_authenticate_credential(&GA->rp, &GA->creds[i])) {
			printf1(TAG_GA, "CRED #%d is invalid" nl, GA->creds[i].credential.id.count);
#ifdef ENABLE_U2F_EXTENSIONS
			if (is_extension_request((uint8_t*)&GA->creds[i].credential.id, sizeof(CredentialId)))
			{
				printf1(TAG_EXT, "CRED #%d is extension" nl, GA->creds[i].credential.id.count);
				count++;
			}
			else
#endif
			...
			if (protection_status != 0) {
				printf1(TAG_GREEN, "skipping protected wrapped credential." nl);
				GA->creds[i].credential.id.count = 0;      // invalidate
			} else {
				count++;
				// add user info if it exists
				if (add_existing_user_info(&GA->creds[i])) {
					printf1(TAG_GREEN, "USER ID SIZE: %d" nl, GA->creds[i].credential.user.id_size);
	...
	printf1(TAG_GA, "qsort length: %d" nl, GA->credLen);
...
static int8_t save_credential_list(uint8_t *clientDataHash,
	...
	if (count) {
		if (count > ALLOW_LIST_MAX_SIZE - 1) {
			printf2(TAG_ERR, "ALLOW_LIST_MAX_SIZE Exceeded" nl);
	...
	printf1(TAG_GA, "saved %d credentials" nl, count);
	return 0;
}
...
uint8_t ctap_end_get_assertion(CborEncoder *map, CTAP_credentialDescriptor *cred, uint8_t *auth_data_buf,
...
#endif
	{
		sigder_sz = ctap_calculate_signature(auth_data_buf, auth_data_buf_sz, clientDataHash, auth_data_buf, sigbuf,
											 sigder, cose_alg);
	}

	printf1(TAG_GREEN, "sigder_sz = %d" nl, sigder_sz);
	...
	if (cred->credential.user.id_size) {
		printf1(TAG_GREEN, "adding user details to output" nl);
...
uint8_t ctap_get_next_assertion(CborEncoder *encoder) {
	...
	if (cred->credential.user.id_size) {
		printf1(TAG_GREEN, "adding user info to assertion response" nl);
		ret = cbor_encoder_create_map(encoder, &map, 4);
	} else {
		printf1(TAG_GREEN, "NOT adding user info to assertion response" nl);
		ret = cbor_encoder_create_map(encoder, &map, 3);
	}
	...
	if (!getAssertionState.user_verified) {
		printf1(TAG_GREEN, "Not verified, nulling out user details on response" nl);
...
uint8_t ctap_cred_mgmt(CborEncoder *encoder, uint8_t *request, int length) {
	...
	if (ret != 0) {
		printf2(TAG_ERR, "error, ctap_parse_cred_mgmt failed" nl);
		return ret;
	}
	...
	if (CM.cmd == CM_cmdRPBegin) {
		...
		printf1(TAG_MC, "RP Begin @%d.  %d total." nl, curr_rp_ind, rp_count);
		...
		// Reset scan
		curr_rk_ind = scan_for_next_rk(0, CM.subCommandParams.rpIdHash);
		printf1(TAG_MC, "Cred Begin @%d.  %d total." nl, curr_rk_ind, rk_count);
	...
	switch (CM.cmd) {
		case CM_cmdMetadata:
			printf1(TAG_CM, "CM_cmdMetadata" nl);
			ret = ctap_cred_metadata(encoder);
			check_ret(ret);
			break;
		case CM_cmdRPBegin:
		case CM_cmdRPNext:
			printf1(TAG_CM, "Get RP %d" nl, curr_rp_ind);
		...
		case CM_cmdRKBegin:
		case CM_cmdRKNext:
			printf1(TAG_CM, "Get Cred %d" nl, curr_rk_ind);
			...
			break;
		case CM_cmdRKDelete:
			printf1(TAG_CM, "CM_cmdRKDelete" nl);
			i = credentialId_to_rk_index(&CM.subCommandParams.credentialDescriptor.credential.id);
			if (i >= 0) {
				ctap_delete_rk(i);
				ctap_decrement_rk_store();
				printf1(TAG_CM, "Deleted rk %d" nl, i);
			} else {
				printf1(TAG_CM, "No Rk by given credId" nl);
				return CTAP2_ERR_NO_CREDENTIALS;
			}
			break;
		default:
			printf2(TAG_ERR, "error, invalid credMgmt cmd: 0x%02x" nl, CM.cmd);
...
uint8_t ctap_get_assertion(CborEncoder *encoder, uint8_t *request, int length) {
	...
	if (ret != 0) {
		printf2(TAG_ERR, "error, parse_get_assertion failed" nl);
		return ret;
	}
	...
	printf1(TAG_GA, "ALLOW_LIST has %d creds" nl, GA.credLen);
	int validCredCount = ctap_filter_invalid_credentials(&GA);

	if (validCredCount == 0) {
		printf2(TAG_ERR, "Error, no authentic credential" nl);
		return CTAP2_ERR_NO_CREDENTIALS;
	} else if (validCredCount > 1) {
		map_size += 1;
	}

	printf1(TAG_GREEN, "2 USER ID SIZE: %d" nl, GA.creds[0].credential.user.id_size);

	if (GA.creds[validCredCount - 1].credential.user.id_size) {
		map_size += 1;
	}
	if (GA.extensions.hmac_secret_present == EXT_HMAC_SECRET_PARSED) {
		printf1(TAG_GA, "hmac-secret is present" nl);
	}
	...
	if (validCredCount < 2 || !getAssertionState.user_verified) {
		printf1(TAG_GREEN, "Only one account, nulling out user details on response" nl);
		memset(&GA.creds[0].credential.user.name, 0, USER_NAME_LIMIT);
	}

	printf1(TAG_GA, "resulting order of creds:" nl);
	int j;
	for (j = 0; j < GA.credLen; j++) {
		printf1(TAG_GA, "CRED ID (# %d)" nl, GA.creds[j].credential.id.count);
	}
...
#endif
	{
		...
		ret = ctap_make_auth_data(&GA.rp, &map, (uint8_t *) &getAssertionState.buf.authData, &auth_data_buf_sz, NULL,
								  &GA.extensions);
	...
	ret = ctap_end_get_assertion(&map, cred, (uint8_t *) &getAssertionState.buf, auth_data_buf_sz,
...
//    calculate shared_secret
	...
	if (memcmp(hmac, pinAuth, 16) != 0) {
		printf2(TAG_ERR, "pinAuth failed for update pin" nl);
		dump_hex1(TAG_ERR, hmac, 16);
		dump_hex1(TAG_ERR, pinAuth, 16);
		return CTAP2_ERR_PIN_AUTH_INVALID;
	}
...
//      validate new PIN (length)
	...
	if (ret < NEW_PIN_MIN_SIZE || ret >= NEW_PIN_MAX_SIZE) {
		printf2(TAG_ERR, "new PIN is too short or too long [%d bytes]" nl, ret);
		return CTAP2_ERR_PIN_POLICY_VIOLATION;
	} else {
		printf1(TAG_CP, "new pin: %s [%d bytes]" nl, pinEnc, ret);
...
uint8_t ctap_add_pin_if_verified(uint8_t *pinTokenEnc, uint8_t *platform_pubkey, uint8_t *pinHashEnc) {
	...
	crypto_sha256_final(pinHashEncSalted);
	if (memcmp(pinHashEncSalted, STATE.PIN_CODE_HASH, 16) != 0) {
		printf2(TAG_ERR, "Pin does not match!" nl);
...
uint8_t ctap_client_pin(CborEncoder *encoder, uint8_t *request, int length) {
	...
	if (ret != 0) {
		printf2(TAG_ERR, "error, parse_client_pin failed" nl);
		return ret;
	}
	...
	switch (CP.subCommand) {
		case CP_cmdGetRetries:
			printf1(TAG_CP, "CP_cmdGetRetries" nl);
		...
		case CP_cmdGetKeyAgreement:
			printf1(TAG_CP, "CP_cmdGetKeyAgreement" nl);
			num_map++;
			...
			break;
		case CP_cmdSetPin:
			printf1(TAG_CP, "CP_cmdSetPin" nl);
			...
			break;
		case CP_cmdChangePin:
			printf1(TAG_CP, "CP_cmdChangePin" nl);
		...
		case CP_cmdGetPinToken:
			...
			printf1(TAG_CP, "CP_cmdGetPinToken" nl);
			if (CP.keyAgreementPresent == 0 || CP.pinHashEncPresent == 0) {
				printf2(TAG_ERR, "Error, missing keyAgreement or pinHashEnc for cmdGetPin" nl);
				return CTAP2_ERR_MISSING_PARAMETER;
			}
		...
		default:
			printf2(TAG_ERR, "Error, invalid client pin subcommand" nl);
			return CTAP1_ERR_OTHER;
	}
...
uint8_t ctap_request(uint8_t *pkt_raw, int length, CTAP_RESPONSE *resp) {
	...
	switch (cmd) {
		case CTAP_MAKE_CREDENTIAL:
			printf1(TAG_CTAP, "CTAP_MAKE_CREDENTIAL" nl);
			timestamp();
			...
			break;
		case CTAP_GET_ASSERTION:
			printf1(TAG_CTAP, "CTAP_GET_ASSERTION" nl);
			timestamp();
			...
			printf1(TAG_DUMP, "cbor [%d]: " nl, resp->length);
			dump_hex1(TAG_DUMP, buf, resp->length);
			break;
		case CTAP_CANCEL:
			printf1(TAG_CTAP, "CTAP_CANCEL" nl);
			break;
		case CTAP_GET_INFO:
			printf1(TAG_CTAP, "CTAP_GET_INFO" nl);
		...
		case CTAP_CLIENT_PIN:
			printf1(TAG_CTAP, "CTAP_CLIENT_PIN" nl);
			...
			break;
		case CTAP_RESET:
			printf1(TAG_CTAP, "CTAP_RESET" nl);
		...
		case GET_NEXT_ASSERTION:
			printf1(TAG_CTAP, "CTAP_NEXT_ASSERTION" nl);
			if (getAssertionState.lastcmd == CTAP_GET_ASSERTION) {
				...
				printf2(TAG_ERR, "unwanted GET_NEXT_ASSERTION.  lastcmd == 0x%02x" nl, getAssertionState.lastcmd);
				status = CTAP2_ERR_NOT_ALLOWED;
			}
			break;
		case CTAP_CBOR_CRED_MGMT:
		case CTAP_CBOR_CRED_MGMT_PRE:
			printf1(TAG_CTAP, "CTAP_CBOR_CRED_MGMT_PRE" nl);
...
void ctap_update_pin(uint8_t *pin, int len) {
	if (len >= NEW_PIN_ENC_MIN_SIZE || len < 4) {
		printf2(TAG_ERR, "Update pin fail length" nl);
		exit(1);
	}
	...
	printf1(TAG_CTAP, "New pin set: %s [%d]" nl, pin, len);
...
uint8_t ctap_decrement_pin_attempts() {
	...
	if (!ctap_device_locked()) {
		...
		ctap_flush_state();
		printf1(TAG_CP, "ATTEMPTS left: %d" nl, STATE.remaining_tries);

		if (ctap_device_locked()) {
			lock_device_permanently();
		}
	} else {
		printf1(TAG_CP, "Device locked!" nl);
		return -1;
	}
	return 0;
}
