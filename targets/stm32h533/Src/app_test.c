...
// l - toggle the Green LED (LD2) mode (OFF, ON, BLINKING_NORMAL, BLINKING_SPECIAL)
// q - reset authenticator (erase storage)
// c - increment the persistent signature counter by 1
// r - app_test_rng_hw
// b - simulate the "reboot" (MCU reset) behavior without the actual reset
// u - simulate presence
// p - print pin remaining attempts
// s - send a USB HID report to the host (a CTAPHID_PING packet)
...
void app_debug_task(void) {
	...
	if (debug_uart_rx == 'q') {
		info_log("reset authenticator (erase storage) ..." nl);
		if (app_storage.erase(&app_storage) == CTAP_STORAGE_OK) {
			ctap_init(&app_ctap);
		} else {
	...
	if (debug_uart_rx == 's') {
		...
		info_log("tud_hid_report result = %d" nl, result);
		return;
	}
...
void app_test_signature_counter(void) {
	...
	ctap_storage_status_t status = app_storage.increment_counter(&app_storage, 1, &counter_new_value);
	const uint32_t t2 = HAL_GetTick();
	if (status != CTAP_STORAGE_OK) {
		error_log(red("app_storage.increment_counter() failed") nl);
	} else {
		info_log("counter_new_value = %" PRIu32 nl, counter_new_value);
	}
	info_log("done in %" PRIu32 " ms" nl, t2 - t1);
}
