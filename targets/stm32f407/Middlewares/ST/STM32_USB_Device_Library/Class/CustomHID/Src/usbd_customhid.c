			...
			switch (req->bRequest) {
				...
				case CUSTOM_HID_REQ_SET_REPORT:
					...
					if (((USBD_CUSTOM_HID_ItfTypeDef *) pdev->pUserData[pdev->classId])->CtrlReqComplete != NULL) {
						...
						((USBD_CUSTOM_HID_ItfTypeDef *) pdev->pUserData[pdev->classId])->CtrlReqComplete(
							req->bRequest,
							req->wLength
						);
					}
...
/**
  ...
  */
#ifdef USE_USBD_COMPOSITE
uint8_t USBD_CUSTOM_HID_SendReport(USBD_HandleTypeDef *pdev, uint8_t *report, uint16_t len, uint8_t ClassId) {
...
static uint8_t USBD_CUSTOM_HID_DataOut(USBD_HandleTypeDef *pdev, uint8_t epnum) {
	...
	// this allows the next USB traffic being NAKed till the end of the application processing
